...
export default function AdminReviewPage() {
  ...
  function handleDraftChange(field, value) {
    setDraftPlant((prev) => ({ ...prev, [field]: value }));
    setSaveMessage("");
  }
  ...
  function handleArrayFieldChange(field, value) {
    ...
    setDraftPlant((prev) => ({ ...prev, [field]: parsed }));
    setSaveMessage("");
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(text || `Failed to save review plant (${res.status})`);
      }
  ...
  async function handlePromote() {
    ...
    try {
      ...
      setSaveMessage("Plant promoted to main catalog.");
    } catch (err) {
      setError(err.message || "Failed to promote plant.");
    } finally {
      setPromoting(false);
    }
  }
  ...
  return (
    <DashboardLayout
      title="Review Queue"
      subtitle="Review staged Trefle imports before promoting them to the main catalog."
      badge="Admin"
    >
      <div className="adminCatalogPage">
        <section className="summaryGrid">
          <div className="summaryCard">
            <span className="summaryLabel">Review Queue</span>
          ...
          <div className="summaryCard">
            <span className="summaryLabel">Selected Review Plant</span>
        ...
        <div className="adminCatalogGrid">
          ...
          <PlantEditorForm
            ...
            onSave={handleSave}
            onReset={resetDraft}
            title="Review Staged Plant"
            actionButtons={
              <button
                type="button"
                className="primaryBtn"
                onClick={handlePromote}
                disabled={promoting || Object.keys(validationErrors).length > 0}
              >
